...
To run *count_tickets.py* in Dumbo cluster we must use the following script:

`spark-submit --num-executors ? --executor-cores ? --executor-memory ? count_tickets.py`
...
`spark-submit --num-executors 6 --executor-cores 5 --executor-memory 10G count_tickets.py`
